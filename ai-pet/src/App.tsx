...
function mergeMessages(base: AgentChatMessage[], incoming: AgentChatMessage[]) {
  ...
  return normalizeMessagesForDisplay(merged);
}
...
export function App() {
  ...
  useEffect(() => {
    const dispose = window.aiPetAppWindow?.onNavigate?.((targetView) => {
      setView(resolveAppView(targetView, "chat"));
    });
    return () => {
      dispose?.();
    };
    ...
    <div className="demo-stage">
      <main className="pet-window" aria-label="AI Pet application window">
        <section className="screen-stack">
          {view === "welcome" ? <WelcomeView profile={profile} state={state} accessory={accessory} outfit={outfit} accent={selectedOutfit.accent} onStart={() => setView("chat")} /> : null}
...
function WelcomeView({
  ...
  return (
    <section className="app-screen welcome-page">
      ...
      <span className="welcome-caption">{profileIdentity.displayName} 已在桌面等待你的下一次互动</span>
    </section>
  );
}
  ...
  async function sendMessage() {
    ...
    try {
      const response = await fetch(apiUrl("/api/agent/chat"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ input, responseMode, threadId, history: nextHistory, context: { ...context, mainThreadId: threadId } }),
